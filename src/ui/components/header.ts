/**
 * Header component for the Figma Token Manager
 */

import { createSidebarToggle } from './sidebarToggle';
...
export function setupHeader(
  ...
  if (!container) {
    ...
    return { 
      ...
      toggleSidebar: () => {},
      setReferenceCounts: () => {},
      viewToggleElement: dummyElement,
      settingsButtonElement: dummyElement
    };
  }
  ...
  // Add sidebar toggle to left section
  headerLeft.appendChild(sidebarToggle.element);
  
  // Add title to left section
  const headerTitle = document.createElement('div');
  headerTitle.className = 'header-title';
  headerTitle.textContent = 'Design Token Manager';
  headerLeft.appendChild(headerTitle);
  
  // Add left section to header content
  headerContent.appendChild(headerLeft);
  
  // Create right section (header controls)
  ...
  // Create visual view button
  const visualViewBtn = document.createElement('button');
  visualViewBtn.className = 'view-toggle-btn active';
  visualViewBtn.setAttribute('data-view', 'visual');
  visualViewBtn.innerHTML = `
  ...
  // Create extract button component
  const extractBtn = createActionButton({
    type: 'extract',
    title: 'Extract Tokens',
    onClick: callbacks.onExtract
  });
  ...
  // Add view buttons to view toggle container
  viewToggle.appendChild(visualViewBtn);
  viewToggle.appendChild(jsonViewBtn);
  
  // Add only action buttons to header controls
  headerControls.appendChild(extractBtn.element);
  headerControls.appendChild(exportBtn.element);
  
  // Add header controls to header content
  ...
  // Add event listeners
  const viewButtons = [visualViewBtn, jsonViewBtn];
  
  viewButtons.forEach(button => {
    button.addEventListener('click', () => {
      const view = button.getAttribute('data-view') as 'visual' | 'json';
  ...
  // Button event listeners are now handled by the action button components
  ...
  return {
    setActiveView: (view: 'visual' | 'json') => {
      ...
      const button = container.querySelector(`.view-toggle-btn[data-view="${view}"]`);
      if (button) {
        (button as HTMLElement).click();
      }
    },
    toggleSidebar: sidebarToggle.toggleSidebar,
    setReferenceCounts: (resolved: number, unresolved: number) => {
      referenceStatus.resolved = resolved;
      referenceStatus.unresolved = unresolved;
      
      settingsModal.updateReferenceCount(resolved, unresolved);
    },
    viewToggleElement: viewToggle,
